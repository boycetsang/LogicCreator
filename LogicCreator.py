...
class Strategy:
    def __init__(self, settings = {}, dataSettings = {}, logicSettings = {}, calSettings = {}, outputSettings = {}):
        self.strategySettings = StrategySettings(settings)
        self.strategyLogic = StrategyLogic(logicSettings)
        self.strategyData = StrategyData(dataSettings, dataNeededDaysBefore = self.strategyLogic.dataNeededDaysBefore)
        self.strategyCalculator = StrategyCalculator(calSettings)
        self.strategyOutput = StrategyOutput(outputSettings)

        
    def __str__(self):
        return '\n'.join([str(self.strategySettings), str(self.strategyData), str(self.strategyLogic), str(self.strategyCalculator), str(self.strategyOutput)])
    
    def dataClean(self, date, timeSeries):
        # timeSeries['datetime'] = pd.to_datetime(timeSeries.iloc[:,1].astype(str) + " " + timeSeries.iloc[:,2].astype(str), format = "%Y%m%d %H:%M:%S.%f")
        # print(timeSeries.columns.values)
        timeSeries['datetime'] = pd.to_datetime(timeSeries.tradeTime, )
        # timeSeries['datetime'] = pd.to_datetime(timeSeries.tradeTime,  format = "%d.%m.%Y %H:%M:%S.%f")
        # timeSeries['datetime'] = timeSeries['datetime']
        timeSeries = timeSeries.set_index('datetime')
        timeSeries.index = timeSeries.index.tz_localize(self.strategySettings.settings['timezoneInRecord'])
        # timeSeries = timeSeries.high * 0.5 + timeSeries.low * 0.5 
        timeSeries = timeSeries.price

        # Data Cleaning
        # timeSeries = timeSeries[(timeSeries.iloc[:,6] > 0) & (timeSeries.iloc[:,7] > 0)]
        # timeSeries = timeSeries.iloc[:,6]*0.5 + timeSeries.iloc[:,7]*0.5

        timeSeries.name = 'price'
        self.indexStartTime = pd.to_datetime(str(date) + " " + self.strategySettings.settings['indexStartTime'], format = "%Y%m%d %H:%M:%S.%f").tz_localize(self.strategySettings.settings['timezoneInIndex']).tz_convert(self.strategySettings.settings['timezoneInRecord'])
        self.indexEndTime = pd.to_datetime(str(date) + " " + self.strategySettings.settings['indexEndTime'], format = "%Y%m%d %H:%M:%S.%f").tz_localize(self.strategySettings.settings['timezoneInIndex']).tz_convert(self.strategySettings.settings['timezoneInRecord'])
        
        self.tradeStartTime = pd.to_datetime(str(date) + " " + self.strategySettings.settings['tradeStartTime'], format = "%Y%m%d %H:%M:%S.%f").tz_localize(self.strategySettings.settings['timezoneInIndex']).tz_convert(self.strategySettings.settings['timezoneInRecord'])
        self.tradeEndTime = pd.to_datetime(str(date) + " " + self.strategySettings.settings['tradeEndTime'], format = "%Y%m%d %H:%M:%S.%f").tz_localize(self.strategySettings.settings['timezoneInIndex']).tz_convert(self.strategySettings.settings['timezoneInRecord'])
        
        self.lunchStartTime = pd.to_datetime(str(date) + " " + self.strategySettings.settings['lunchStartTime'], format = "%Y%m%d %H:%M:%S.%f").tz_localize(self.strategySettings.settings['timezoneInIndex']).tz_convert(self.strategySettings.settings['timezoneInRecord'])
        self.lunchEndTime = pd.to_datetime(str(date) + " " + self.strategySettings.settings['lunchEndTime'], format = "%Y%m%d %H:%M:%S.%f").tz_localize(self.strategySettings.settings['timezoneInIndex']).tz_convert(self.strategySettings.settings['timezoneInRecord'])
        # print(timeSeries.index.min(), timeSeries.index.max())

        grouped = timeSeries.groupby(level=0)
        timeSeries = grouped.last()
        timeSeries  = timeSeries [(timeSeries.index > self.indexStartTime) & (timeSeries.index < self.indexEndTime) & ((timeSeries.index > self.lunchEndTime) | (timeSeries.index < self.lunchStartTime))]   
        # treating the insufficient data case
        if len(timeSeries) != 0:
            timeSeries[self.indexEndTime] = timeSeries[(timeSeries.index <= self.indexEndTime)].iloc[-1] 
        # print(timeSeries)
        return timeSeries
    ...
    def evalStrat(self, silence = False):
        ...
        date_cnt = 0
        date_total = len(set(self.strategyData.tradeDateRange.date).intersection(set(self.strategyData.dateFileDict.keys())))
        if not silence:
        ...
        for date in self.strategyData.tradeDateRange:
            ...
            self.strategyData.timeSeries = pd.read_csv(self.strategyData.dateFileDict[date], na_values = ['NA'])
#            print(self.strategyData.timeSeries.ix[:,2])
            
            self.strategyData.timeSeries = self.dataClean(date, self.strategyData.timeSeries)
            
            self.strategyData.timeSeriesTick = self.strategyData.timeSeries
            self.strategyData.timeSeriesTrade = self.strategyData.timeSeries.resample(str(int(float(self.strategySettings.settings['interval'])*60))+'S').first()
            if self.strategyData.timeSeriesTrade.empty:
                    date_cnt += 1
                    continue
            self.strategyLogic.performComputeForDay(self, self.strategyData.timeSeriesTick, self.strategyData.timeSeriesTrade)
            ...
            for ind, val in self.strategyData.timeSeriesTrade.iteritems():
                ...
                for position in self.strategyCalculator.openPositionList:
                     if position.type == 'buy' and (self.strategyLogic.getBuyExitCondition(self, position, ind)):
                         position.close(ind, val)
                         self.strategyLogic.getExitReport(self, position)
                     if position.type == 'sell' and (self.strategyLogic.getSellExitCondition(self, position, ind)): 
                         position.close(ind, val)
                         self.strategyLogic.getExitReport(self, position)
    ...
    def evalStratDay(self, date):
        ...
        # print(self.strategyCalculator.positionList)
        import copy
    ...
    def outputGraphsParallel(self, silence = False, num_core = 0):
        ...
        Parallel(n_jobs=num_cores, verbose=51)(delayed(self.strategyOutput.outputGraphsDay)(self, date) for date in self.strategyData.tradeDateRange)

            
    def __add__(self, other):
        self.strategyCalculator.positionList = self.strategyCalculator.positionList + other.strategyCalculator.positionList
        return self
...
#    logicSettings[PnLLogic] = {}
    ...
    # logicSettings[BounceLogic] = dict(
    #                                   fast_period=400, 
    #                                   slow_period=800,
    #                                   lag_time=45,
    ...
    #                                   takeProfit=2, 
    #                                   stopLoss=3, 
    #                                   beta_threshold_upper_const=0.003, 
    #                                   beta_threshold_lower_const=-0.003, 
    #                                   filter_type='momentum', 
    #                                   std_window=30, 
    #                                   beta_threshold_upper_ewma_coeff=0, 
    #                                   beta_threshold_lower_ewma_coeff=0, 
    #                                   beta_threshold_upper_std_coeff=0, 
    #                                   beta_threshold_lower_std_coeff=0, 
    #                                   movement_period=30, 
    #                                   takeProfitFrac=0.4,
    #                                   stopLossFrac=0.15, 
    #                                   ewma_delta_threshold=2,
    #                                   beta_long_multiplier=15,
    #                                   beta_long_threshold=0.006
    #                                   )

    logicSettings[BasicLogic] = dict(takeProfit = 7.65, 
                                     stopLoss = 7.65, 
                                     totalExposure = 10, 
                                     trailing = 0)
    logicSettings[RegLogic] = dict(period = 475, 
                                   delta_period = 15, 
                                   beta_threshold = 0.0005, 
                                   resamplePeriod = 5, 
                                   filter_type = 'momentum')
    logicSettings[RSILogic] = dict(filterStart = 10, 
                                   filterEnd = 52, 
                                   filterWindow = 372, 
                                   # windowType = None, 
                                   reverse = False)
    ...
    strategy = Strategy(settings = settings, dataSettings = dataSettings, logicSettings = logicSettings, calSettings = calSettings, outputSettings = outputSettings)
...
#   Get realPnL for quick review
    ...
    print("Benchmark Time from " + settings['dateStart'] + " to " + settings['dateEnd'] + " is "  + str(time.time() - start) + ' seconds.')
