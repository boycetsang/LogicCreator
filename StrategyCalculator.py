...
class StrategyCalculator:
    ...
    def __str__(self):
        return os.linesep.join(["StrategyCalculator: " + str(self.calSettings)] + ['Current Positions: '] + [str(position) for position in self.openPositionList])
    ...
    class Position(object):
        ...
        def __str__(self):
            return os.linesep.join(["Position: " + str(self.type),  "Open at time " + str(self.openTime) , 'Open at price ' + str(self.openPosition),  "Close at time " + str(self.closeTime) , 'Close at price ' + str(self.closePosition)] )
        ...
        def close(self, closeTime, closePosition):
            ...
            if self.type == 'buy':
                 ...
                 self.realPnL = self.PnL - self.strategy.strategyCalculator.calSettings['slippery']
            if self.type == 'sell':
                 self.strategy.strategyCalculator.sellCnt -= 1
                 self.PnL = (-closePosition + self.openPosition)*self.tradeAmount
                 self.realPnL = self.PnL - self.strategy.strategyCalculator.calSettings['slippery']
