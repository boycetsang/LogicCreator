...
class StrategyData:
    def __init__(self, dataSettings, dataNeededDaysBefore = []):
        ...
        self.dateStart = pd.to_datetime(dataSettings['dateStart'], format = '%Y%m%d' ).date()
        ...
        for date in self.tradeDateRange:
            pathName = os.path.join(dataSettings['dataPath'], date.strftime('%Y%m%d') + ".csv")
            if os.path.isfile(pathName):
                self.dateFileDict[date.date()] = pathName            
            
            for day in dataNeededDaysBefore:
                pathName = os.path.join(dataSettings['dataPath'], (date-timedelta(days=day)).strftime('%Y%m%d') + ".csv")
                if os.path.isfile(pathName):
