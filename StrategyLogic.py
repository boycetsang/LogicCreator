class StrategyLogic:
    ...
    def getBuyEntranceCondition(self, strategy, ind):
        ...
        for logicSetting in self.logicCollection:
            if logicSetting.BUY_ENTRANCE_LOGIC_OPERATION == 'or':
                buyEntranceCondition = buyEntranceCondition or logicSetting.addBuyEntranceCondition(strategy, ind)
            else:
                buyEntranceCondition = buyEntranceCondition and logicSetting.addBuyEntranceCondition(strategy, ind)
    ...
    def getBuyExitCondition(self, strategy, position, ind):
        ...
        for logicSetting in self.logicCollection:
            if logicSetting.BUY_EXIT_LOGIC_OPERATION == 'or':
                buyExitCondition = buyExitCondition or logicSetting.addBuyExitCondition(strategy, position, ind)
            else:
                buyExitCondition = buyExitCondition and logicSetting.addBuyExitCondition(strategy, position, ind)
    ...
    def getSellEntranceCondition(self, strategy, ind):
        ...
        for logicSetting in self.logicCollection:
            if logicSetting.SELL_ENTRANCE_LOGIC_OPERATION == 'or':
                sellEntranceCondition = sellEntranceCondition or logicSetting.addSellEntranceCondition(strategy, ind)
            else:
                sellEntranceCondition = sellEntranceCondition and logicSetting.addSellEntranceCondition(strategy, ind)
    ...
    def getSellExitCondition(self, strategy, position, ind):
        ...
        for logicSetting in self.logicCollection:
            ...
            if logicSetting.SELL_EXIT_LOGIC_OPERATION == 'or':
                ...
                sellExitCondition = sellExitCondition or logicSetting.addSellExitCondition(strategy, position, ind)
            else:
                sellExitCondition = sellExitCondition and logicSetting.addSellExitCondition(strategy, position, ind)
    ...
    def performComputeForDay(self, strategy, timeSeriesTick, timeSeriesTrade):
        for logicSetting in self.logicCollection:
            self.extraTimeSeries.update(logicSetting.computeForDay(strategy, timeSeriesTick, timeSeriesTrade))
