...
class StrategyOutput:
    ...
    def outputToFile(self, strategy, silence=False, useNumber=0):
        ...
        if not outputTable.empty:
            ...
            outputTable.closeTime = pd.to_datetime(outputTable.closeTime, format = "%Y%m%d %H:%M:%S.%f")
            outputTable['closeDate'] = outputTable.closeTime.dt.strftime('%Y%m%d')
            outputTable.closeTime = outputTable.closeTime.dt.time
            outputTable.openTime = pd.to_datetime(outputTable.openTime, format = "%Y%m%d %H:%M:%S.%f")
            ...
            if not silence:
                if useNumber:
                    outputTable.to_csv('SOLN_OUTPUT_' + str(useNumber) + '.csv', index = None)
                else:
    ...
    def updateProgress(self, progress):
        """ Simple progress bar"""
        sys.stdout.write('\r[{0}] {1:.0f}%'.format('#'*int(progress*10) + '.'*int(10 - progress*10), progress*100))
    ...
    def outputGraphs(self, strategy, dates = []):
        ...
        if dates == [] and self.outputSettings['graphOutput']:        
            import pathlib
            pathlib.Path(self.outputSettings['graphOutput']).mkdir(parents=True, exist_ok=True) 
            for date in strategy.strategyData.tradeDateRange: 
                date = date.date()
                print(date)
                if not date in strategy.strategyData.dateFileDict.keys():
                    continue
                timeSeries = pd.read_csv(strategy.strategyData.dateFileDict[date], index_col = None, na_values = ['NA'])
                timeSeries = strategy.dataClean(date, timeSeries)

                timeSeriesTick = timeSeries

                timeSeriesTrade = timeSeries.resample(str(int(float(strategy.strategySettings.settings['interval'])*60))+'S').first()
                ...
                strategy.strategyLogic.performComputeForDay(strategy, timeSeriesTick, timeSeriesTrade)
                ax1 = plt.gca()
                ...
                ax1.legend(loc=3)
                ax2.legend(loc=4)
                positionListForThisDate = [position for position in strategy.strategyCalculator.positionList if position.openTime.date() == date]
                    ...
                    if position.type == 'buy':
                        if hasattr(position, 'movement') and position.movement:
                            if position.closePosition > position.openPosition:
                                plt.plot([position.openTime, position.openTime], [position.openPosition, position.openPosition + position.movement], '-.g')
                            else:
                                plt.plot([position.openTime, position.openTime], [position.openPosition, position.openPosition + position.movement], '-.r')
                        if position.closePosition > position.openPosition:
                            plt.plot(position.openTime, position.openPosition, 'og')
                            plt.plot([position.openTime, position.closeTime], [position.openPosition, position.closePosition], '--g', linewidth = 1)
                        else:
                            plt.plot(position.openTime, position.openPosition, 'or')
                            try:
                                plt.plot([position.openTime, position.closeTime], [position.openPosition, position.closePosition], '--r', linewidth = 1)
                            except:
                                pass
                ...
                graphPath = os.path.join(os.getcwd(), self.outputSettings['graphOutput'], str(date) + ".png")
    ...
    def getRealPnLPerTradedDay(self, strategy):
        ...
        if self.outputTable.empty:
            ...
            realPnLPerTradedDay = self.outputTable.groupby('openDate').apply(lambda x: x.realPnL.sum()).mean()
    ...
    def getRealPnLPerDay(self, strategy):
        ...
        if self.outputTable.empty:
            ...
            realPnLPerDay = self.outputTable.realPnL.sum()/(strategy.strategyData.dateEnd - strategy.strategyData.dateStart).days
    ...
    def getRealPnLStdDay(self, strategy):
        ...
        if self.outputTable.empty:
            ...
            realPnLPerStddDay = self.outputTable.groupby('openDate').apply(lambda x: x.realPnL.sum()).std()
    ...
    def getRealPnLStdMonth(self, strategy):
        ...
        if self.outputTable.empty:
            ...
            self.outputTable['openMonth'] = pd.to_datetime(self.outputTable.openDate, format = "%Y%m%d").apply(lambda x: x.month)
            
            realPnLPerStddDay = self.outputTable.groupby('openMonth').apply(lambda x: x.realPnL.sum()).std()
            return realPnLPerStddDay
    ...
    def getRealPnLMaxDrawBackInDay(self, strategy):
        ...
        if self.outputTable.empty:
            ...
            realPnLPerMaxDrawBackInDay = self.outputTable.groupby('openDate').apply(lambda x: (np.nancumsum(x.realPnL.iloc[::-1]) - pd.Series(np.nancumsum(x.realPnL.iloc[::-1])).cummax()).min())
    ...
    def getRealPnLMaxDrawBack(self, strategy):
        ...
        if self.outputTable.empty:
            ...
            realPnLPerMaxDrawBack = (np.nancumsum(self.outputTable.realPnL.iloc[::-1]) - pd.Series(np.nancumsum(self.outputTable.realPnL.iloc[::-1])).cummax()).min()
    ...
    def getRealWinDayRatio(self, strategy):
        ...
        if self.outputTable.empty:
            ...
            winDayRatio = (self.outputTable.groupby('openDate').apply(lambda x: x.realPnL.sum() >= 0)).sum()/len(self.outputTable['openDate'].value_counts())
            return winDayRatio
