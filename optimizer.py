...
def obtainSettings():
    ...
    readInTable = pd.read_csv('optSettings.csv', index_col = 0, keep_default_na=False, na_values = ['NA'])
    ...
    for logicClass in logicClasses:
        ...
        for ii in range(len(args)):
            if args[ii] in defaultValues.index:
                logicSettings[logicClass].update({args[ii]: try_convert(defaultValues[args[ii]])})
                ...
                if not logicClass in logicCollection:
                    ...
                    logicIndexList.append(defaultValuesRI[defaultValuesRI.setting == args[ii]].index.tolist()[0])
            else:
                print(args[ii] , 'settings for', logicClass.__name__, 'is not found, loading its default values and turning the module off ...')
                logicSettings[logicClass]['status'] = False
#                settings[args[ii]] = inspect.getargspec(logicClass).defaults[ii]
                logicSettings[logicClass].update({args[ii]: inspect.getargspec(logicClass).defaults[ii]})
#    pd.DataFrame(settings, index = ['default']).T.to_csv('defaultValues.csv', index_label = 'setting')
    ...
    logicSettings =  OrderedDict((logic, logicSettings[logic]) for logic in logicCollection)
...
#    optVars = OrderedDict(optTable.T.to_dict())
    optVars = OrderedDict([(row.name, OrderedDict([('min', row['min']), ('max', row['max']), ('no_of_steps', row['no_of_steps'])])) for i, row in optTable.iterrows()])
...
#    print(optVars, optVarValue)
    ...
    for optVar in optVars.keys():
        ...
        optVarValue[optVar] = round(min_val + int(individual[cnt]/100*step_cnt)/step_cnt*(max_val-min_val),7)
        cnt += 1
    ...
    for logicClass in logicSettingsExecute.keys():
        logicSettingsExecute[logicClass] = {k.replace(logicClass.__name__.replace('Logic', '')+"_", ''): v for k, v in logicSettings[logicClass].items()}
        logicSettingsExecute[logicClass].pop('status')

            
        
#    print(logicSettingsExecute)
    Strategy = LogicCreator.Strategy(settings = settings, dataSettings = dataSettings, logicSettings = logicSettingsExecute, calSettings = calSettings, outputSettings = outputSettings)
    # strategy.evalStrat()
...
#    print(strategy)
    ...
    realPnLMaxDrawBackInDay = strategy.strategyOutput.getRealPnLMaxDrawBackInDay(strategy)
    ...
    if lock:
        with counter.get_lock():
            counter.value += 1
        writeStr = ','.join([str(counter.value)] + [str(optVarValue[x]) for x in optVarValue.keys()] + metric + [str(fitness)])
    else:
        writeStr = ','.join([str(soln_cnt)] + [str(optVarValue[x]) for x in optVarValue.keys()] + metric + [str(fitness)])
...
def main(pool = None, lock = None, optSettings =  OrderedDict([('indpb', 0.15), ('tournsize', 3), ('noOfCore', multiprocessing.cpu_count() - 1), ('pop_n', 100), ('ngen', 9), ('mutpb', 0.1), ('cxpb', 0.5) ])):
    random.seed(64)
    ...
    pop, log = algorithms.eaSimple(pop, toolbox, cxpb=optSettings['cxpb'], mutpb=optSettings['mutpb'], ngen=optSettings['ngen'], 
...
#    importPackages()
    ...
    optSettings =  OrderedDict([('noOfCore', multiprocessing.cpu_count() - 1), ('indpb', 0.15), ('tournsize', 3), ('pop_n', 200), ('ngen', 90), ('mutpb', 0.1), ('cxpb', 0.5) ])
...
#    printLogicSettings(logicSettings)
    ...
    while not go:
        ...
        options = ['Change settings', 'Modify existing optimization variables', 'Add new optimization variables', 'Enable/Disable Logic modules', 'Change settings of Logic modules', 'Change Optimization settings', 'Calibration Re-run/Verification Run', 'Run Program']
        ...
        if choice == 0:
            ...
            newVal = input("Change the value of " + str(settingKeys[choice]) +  " to ... ")
            ...
            choice = textMenu(optVarsKeys, title = 'CHOOSE OPTIMIZATION VARIABLE TO CHANGE')
            optVarKey = optVarsKeys[choice]
            optVarSettings = optVars[optVarKey]
            optVarSettingsKeys = list(optVarSettings.keys())
            optVarSettingsKeysShow = [optVarSettingsKey + ": " + str(optVarSettings[optVarSettingsKey]) for optVarSettingsKey in optVarSettingsKeys]
            ...
            if choice == len(optVarSettingsKeys) - 1:
                ...
                newVal = input("Change the value of " + str(optVarSettingsKeys[choice]) +  " to ... ")
                newVal = try_convert(newVal)            
                optVarSettings[optVarSettingsKeys[choice]] = newVal
                
        elif choice == 2:
            choice = textMenu(['Add general setting type', 'Add logic setting type'], title = 'CHOOSE OPTIMIZATION VARIABLE TO CHANGE')
            if choice == 0:
#                print ("Showing non-optimized variables ...")
                ...
                if not settingKeys[choice] in list(optVars.keys()):
                    ...
                    newVal = input("Min value of " + str(settingKeys[choice]) +  " is ... ")
                    min_val = try_convert(newVal)
                    newVal = input("Max value of " + str(settingKeys[choice]) +  " is ... ")
                    max_val = try_convert(newVal)
                    newVal = input("Step count of " + str(settingKeys[choice]) +  " is ... ")
                    step_cnt = int(newVal)      
                    optVars[settingKeys[choice]] = {'min': min_val, 'max': max_val, 'no_of_steps': step_cnt}
                else:
                    print(settingKeys[choice] + ' is already in the opitmization varaible set!')
            else:
                logicClasses = [logicClass  for logicClass in logicSettings.keys()]
                logicClassesShow = [logicClass.__name__ + ' Current status :\t' + str(logicSettings[logicClass]['status'])  for logicClass in logicSettings.keys()]
            ...
            if choice != -1:
                logicSettings[logicClasses[choice]]['status'] = not logicSettings[logicClasses[choice]]['status']
            ...
            print("This run was carried out from " + settings['dateStart'] + " to " + settings['dateEnd']) 
            choice = textMenu(['Calibration Re-run (Parallel)', 'Calibration Re-run', 'Verification Run (Parallel)', 'Verification Run', 'Calibration and Verification Run (Parallel)'])
            outputSettings['graphOutput'] = settings['graphOutput']   
            ...
            while doCaliToo:
                doCaliToo = False
                if choice == 0:
                    outputSettings['summaryOutput'] = os.path.join(mdl_dir, 'Summary_calibration.csv')
                ...
                if choice == 2 or choice == 4:
                    ...
                    outputSettings['summaryOutput'] = os.path.join(mdl_dir, 'Summary_verification.csv')
                ...
                for setting in settings.keys():
                    ...
                    for logicClass in logicClasses:
                        ...
                        if setting.startswith(logicName):
                            appearedSettings[setting.replace(logicName+"_", "")] = try_convert(settings[setting])
                ...
                strategy = LogicCreator.Strategy(settings = settings, dataSettings = dataSettings, logicSettings = logicSettings, calSettings = calSettings, outputSettings = outputSettings)
                ...
                performance_dict = {'realPnLPerTradedDay': realPnLPerTradedDay,  
                ...
                metric.append(str(realPnLMaxDrawBack))
                metric.append(str(realWinDayRatio))
                print( '\t'.join(['TotalRealPnL', 'RealPnLPerTradedDay', 'RealPnLPerDay',
                       'RealPnLStdDay', 'RealPnLStdMonth', 'PnLPerTrade', 'RealPnLMaxDrawBackInDay', 'RealPnLMaxDrawBack', 'RealWinDayRatio']) + '\n')

                print('\t'.join( metric))
                if choice == 0 or choice == 1:
                    performance_df.to_csv(os.path.join(mdl_dir, 'Performance_calibration.csv'))
                    strategy.strategyOutput.outputGraphs(strategy)
                else:
                    performance_df.to_csv(os.path.join(mdl_dir, 'Performance_verification.csv'))
                if doCaliToo:
                    choice = 0
            ...
            print('## CAUTION: The order of Logic modules is important due to possible combination of and/or conditions! ##')
            print('To change the order of the Logic modules, go back or change the order in optSettings.csv')
            print()
            print('-=-'*15)  
            print(' > '.join([logicClass.__name__ for logicClass in logicSettings.keys() if logicSettings[logicClass]['status']]))
            print('-=-'*15) 
            print()
            ...
            if optVars:
                printOptVars(optVars)
                choice = input('Here is the optimization variable setting ... Are you sure to continue? [(Y)/N]')
                ...
                print('No Optimization variable! This is a verification job. Summary output will be written. 1 CPU will be used')
                go = True
                veri = True
                optSettings['noOfCore'] = 1
    ...
    toolbox.register("evaluate", evalOneMax, settings = settings, logicSettings = logicSettings, optVars = optVars)
    toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_bool, len(optVars.keys()))
#    toolbox.register("individual", tools.initRepeat, creator.Individual, toolbox.attr_bool, 1)
    ...
    header = ','.join(['SolutionID'] + list(optVars.keys()) + ['TotalRealPnL', 'RealPnLPerTradedDay', 'RealPnLPerDay',
     'RealPnLStdDay', 'RealPnLStdMonth', 'PnLPerTrade', 'RealPnLMaxDrawBackInDay', 'RealPnLMaxDrawBack', 'RealWinDayRatio', 'fitness']) + '\n'
    print(header)
...
#    init()
    if optSettings['noOfCore'] > 1:
        ...
        pool = multiprocessing.Pool(optSettings['noOfCore'],initializer=init, initargs=(l,counter,))
